...
import {
	addRoute,
	fetchCollectorRoutes,
} from "../../redux/thunks/collectorsThunk";
...
function PublishRoutes() {
	...
	const addLocation = (location_name) => {
		...
		_routes[activeTab] = [
			..._routes[activeTab],
			{ collectionpoint: { Location: { name: location_name } } },
		];
	...
	const deleteLocation = (idx) => {
		...
		if (point?.point_id && point?.route_id) {
			setDeletedPoints([
				...deletedPoints,
				point.collectionpoint.Location.location_id,
			]);
		}
	};
